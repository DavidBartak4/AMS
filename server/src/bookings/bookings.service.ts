...
export class BookingService {
  ...
  async createBooking(body: CreateBookingBodyDto) {
    try {
      await this.sendBookingConfirmationByMail(body)
    } catch (err) {
      if (err.message === "Cannot send confirmation because host mail auth is not configured") {
        throw new InternalServerErrorException("Booking cannot be created, please contact web administrator")
      } else {
  ...
  private async sendBookingConfirmationByMail(booking): Promise<void> {
    const configuration = await this.configurationService.getConfiguration()
    const mailConfiguration = configuration.mail
    if (!mailConfiguration.mailUsername || !mailConfiguration.mailPassword) { throw new Error("Cannot send confirmation because host mail auth is not configured") }
